...
module.exports = function registerRecordingRoutes(app, ctx) {
  ...
  app.options("/recordings/*", ctx.auth.recordingCors);
  app.post("/recordings/start", ctx.auth.recordingCors, express.json(), (req, res) => {
  ...
  app.post("/recordings/events", ctx.auth.recordingCors, express.json(), (req, res) => {
  ...
  app.post("/recordings/end", ctx.auth.recordingCors, express.json(), (req, res) => {
    const sessionId = String(req.body?.sessionId || "").trim();
    const session = ctx.recordingSessions.get(sessionId);
    if (!session) return res.status(404).json({ error: "Session not found" });
  ...
  app.get("/recordings/check/:sessionId", (req, res) => {
  ...
  (function(){
    ...
    stopBtn.onclick = function() {
      post("/recordings/events", { sessionId: SESSION_ID, events: events }).then(function() {
        return post("/recordings/end", { sessionId: SESSION_ID });
  ...
  fetch("/recordings/check/"+encodeURIComponent(sessionId)).then(function(r){return r.json();}).then(function(d){
  ...
  // S7: the SPA no longer lives inside the API image. /record is a lightweight
  // bookmarklet page served by the API itself for the browser recording flow.

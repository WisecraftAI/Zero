...
| Also touch | Orchestrator worker · folder `apps/orchestrator/` · npm `@zero/orchestrator` · skill `/zero-orchestrator` |

## Scope

- Move Express route handlers out of `apps/api/server.js` into `apps/api/src/routes/` (or `apps/api/routes/`).
- `processRun` / stage walk must not be defined in `apps/api/server.js`. Require it from `@zero/orchestrator`.
...
- [x] At least two route modules under `apps/api/src/routes/` or `apps/api/routes/`
- [x] `apps/api/server.js` does not contain `async function processRun`
- [x] Orchestrator worker owns the DAG (`processRun` / `stageKeys` walk in `apps/orchestrator/`)

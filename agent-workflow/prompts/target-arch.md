...
4. `apps/api/server.js` — HTTP API composition root (routes in `src/routes/`; Chromium in `@zero/executor`)
5. `packages/*` — `@zero/cloud` · `@zero/db` · `@zero/domain` · `@zero/locators` · `@zero/builders` · `@zero/analyzer`
6. `web/` — React UI
7. `agent-workflow/` — autonomous loop, milestone specs, progress.json
...
Distinguish **current runtime** (one Node process: HTTP API + orchestrator + Playwright, Maps, optional Postgres) from **target** (stateless API · queue orchestrator · ephemeral Playwright farm · required Postgres + object store + Redis).

## North-star shape (do not redesign)
Three tiers, one contract:
- **HTTP API** — folder `apps/api/` · npm `@zero/api` · skill `/zero-api`. Auth, run intake, status/SSE, signed URLs. Never runs Chromium.
- **Orchestrator worker** — folder `apps/orchestrator/` · npm `@zero/orchestrator` · skill `/zero-orchestrator`. BA · Manual · Automation · Manager · Delivery, LLM calls, DAG state. Consumes `runs.requested`.
- **Playwright executor** — folder `apps/executor/` · npm `@zero/executor` · skill `/zero-executor`. Ephemeral Playwright jobs on `execution.requested`; screenshots/traces to object store.
...
P2: disconnected LLM UI, unfinished IDE/projects vision, monolith split as needed for workers — keep focused; no drive-by refactors of all of `apps/api/server.js`.
...
2. If the user named a workspace, read `agent-workflow/prompts/repos/<name>.md` and use that skill (`/zero-api`, `/zero-web`, …). Always spell folder, npm package, and Cursor skill.
3. M1–M7 capability probes are green. Remaining work is V3 packaging **S3–S6** — do not re-implement M1–M4.
4. Small, reviewable PRs; update `zero-docs` status when something flips done / not-done.
5. Prefer adapters + interfaces first, then one concrete provider (`local` or AWS) so local/dev still runs.
6. When done, run `npm run workflow:verify -- --milestone S3` (or the named S step). Ask before destructive migrations or public API breaks.
...
Implement the next unfinished packaging step (S3–S6). Read `prompts/packaging.md`. Ask before destructive migrations or changing public API contracts.

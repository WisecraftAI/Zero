...
async function initRunsTables(pool) {
  ...
  await pool.query(TABLE_AI_CACHE);
...
async function upsertRun(pool, run) {
  ...
  await pool.query(
    `INSERT INTO qa_runs (
       ...
       manager_report_json, delivery_report_json, cms_signal_json, updated_at
...
async function listRuns(pool, limit = 20) {
  const r = await pool.query(
    "SELECT id, project_id, status, input_json, created_at, updated_at FROM qa_runs ORDER BY created_at DESC LIMIT $1",
    [limit]
  );
  return r.rows;
}
...
async function getAICache(pool, cacheKey) {
  const r = await pool.query("SELECT result_json FROM ai_generation_cache WHERE cache_key = $1", [cacheKey]);
  return r.rows[0]?.result_json || null;
}

async function setAICache(pool, { cacheKey, host, provider, model, resultJson }) {
  await pool.query(
    `INSERT INTO ai_generation_cache (cache_key, host, provider, model, result_json)
     VALUES ($1,$2,$3,$4,$5::jsonb)
     ON CONFLICT (cache_key) DO UPDATE SET result_json = EXCLUDED.result_json, created_at = NOW()`,
    [cacheKey, host || null, provider || null, model || null, JSON.stringify(resultJson)]
  );
}
...
async function initUserTables(pool) {
  await pool.query(TABLE_USERS);
...
async function createUser(pool, { email, passwordHash, name }) {
  const r = await pool.query(

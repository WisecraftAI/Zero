...
const { detectDomain, getDomainConfig, getSelectors } = require("./ecommerceSelectors");
...
const WEBSITE_TYPES = {
  ...
  CMS_ADMIN: {
    name: 'CMS/Admin Portal',
    indicators: ['admin', 'dashboard', 'cms', 'content management', 'settings', 'users', 'analytics', 'reports', 'configure', 'publish'],
    urlPatterns: ['admin', 'cms', 'portal', 'dashboard', 'manage', 'console', 'control', 'backstage'],
    testPriorities: ['Authentication', 'Dashboard Load', 'Navigation', 'CRUD Operations', 'Permissions'],
    criticalFlows: ['Login Flow', 'Dashboard Navigation', 'Content Management', 'User Management']
  },
  WEBAPP_SPA: {
    name: 'Web Application (SPA)',
    indicators: ['app', 'data-react', 'ng-app', '__next', 'application', 'dashboard', 'workspace', 'project'],
    urlPatterns: ['app', 'web-ui', 'webapp', 'portal', 'platform', 'console', 'ui', 'client'],
    testPriorities: ['Page Load', 'Navigation', 'Components', 'State Management', 'User Interactions'],
    criticalFlows: ['App Initialization', 'Navigation Flow', 'Core Feature Access']
  },
  STAGING_TEST: {
    name: 'Staging/Test Environment',
    indicators: ['staging', 'test', 'demo', 'qa', 'uat', 'dev', 'sandbox'],
    urlPatterns: ['stag', 'test', 'qa', 'uat', 'dev', 'sandbox', 'preview', 'demo'],
    testPriorities: ['Environment Validation', 'Feature Testing', 'Integration Points', 'Data Loading'],
    criticalFlows: ['Environment Access', 'Feature Validation', 'Data Flow Testing']
  },
  GENERIC: {
    name: 'Website',
    indicators: [],
...
function detectUserFlows(websiteType, elements, forms, pageStructure) {
  ...
  switch (websiteType.type) {
    case 'RETAIL_STORE':
      ...
      // Store Timings Flow
      flows.push({
        name: 'Store Timings Verification',
        priority: 'High',
        description: 'Verify store operating hours are displayed correctly',
        steps: [
          { action: 'navigate', target: 'Branch/Store page', description: 'Navigate to branch information' },
          { action: 'verify', target: 'Store Timings', description: 'Verify store operating hours are displayed' },
          { action: 'verify', target: 'Opening Hours', description: 'Verify opening time is shown' },
          { action: 'verify', target: 'Closing Hours', description: 'Verify closing time is shown' }
        ],
        assertions: [
          'Store timings section is visible',
          'Opening hours are clearly displayed',
          'Closing hours are clearly displayed',
          'Weekend/holiday hours are indicated if different'
        ]
      });
      ...
      flows.push({
        ...
        priority: 'High',
        description: 'Verify contact information is accessible',
        steps: [
          { action: 'navigate', target: 'Contact Page', description: 'Navigate to contact information page' },
          { action: 'verify', target: 'Phone Numbers', description: 'Verify phone numbers are displayed' },
          { action: 'verify', target: 'Email Address', description: 'Verify email address is shown if available' },
          { action: 'verify', target: 'Physical Address', description: 'Verify physical address is displayed' }
        ],
        assertions: [
          'Contact page is accessible',
          'Phone numbers are displayed',
          'Address information is complete',
          'Contact form works if available'
        ]
      });

      // Offers/Promotions Flow (if promotional content detected)
      if (hasCategory('SLIDER') || hasCategory('CARDS')) {
        flows.push({
          name: 'Offers and Promotions Display',
          priority: 'Medium',
          description: 'Verify promotional content and offers are displayed',
          steps: [
            { action: 'navigate', target: 'Homepage', description: 'Navigate to homepage' },
            { action: 'verify', target: 'Promotional Banners', description: 'Verify promotional banners/sliders are visible' },
            { action: 'verify', target: 'Current Offers', description: 'Verify current offers are displayed' },
            { action: 'interact', target: 'Offer Details', description: 'Click on offer to view details if applicable' }
          ],
          assertions: [
            'Promotional banners load correctly',
            'Current offers are visible',
            'Offer details are clear',
            'Banner slider/carousel works if present'
          ]
        });
      }
      break;
  ...
  // Media Content Flow (if media detected) - Domain-aware
  if (hasCategory('MEDIA') && !flows.some(f => f.name.includes('Video'))) {
    const isOTT = websiteType.type === 'OTT_STREAMING';
    const isRetailOrCorporate = ['RETAIL_STORE', 'CORPORATE', 'HEALTHCARE_PHARMA'].includes(websiteType.type);

    if (isOTT) {
      // Full video playback flow for OTT platforms
      flows.push({
        name: 'Video Content Playback',
        priority: 'Critical',
        description: 'Verify video content loads and plays with full controls',
        steps: [
          { action: 'navigate', target: 'Content Page', description: 'Navigate to video content' },
          { action: 'verify', target: 'Video Player', description: 'Verify video player is visible' },
          { action: 'interact', target: 'Play Button', description: 'Click play button' },
          { action: 'verify', target: 'Playback', description: 'Verify video starts playing' },
          { action: 'interact', target: 'Pause Button', description: 'Click pause button' },
          { action: 'verify', target: 'Paused State', description: 'Verify video is paused' }
        ],
        assertions: [
    ...
    } else if (isRetailOrCorporate) {
      // For retail/corporate - Image Gallery flow, NOT video playback
      flows.push({
        name: 'Image Gallery Display',
        priority: 'Medium',
        description: 'Verify product images and promotional media display correctly',
        steps: [
          { action: 'navigate', target: 'Gallery Section', description: 'Navigate to image gallery/product images' },
          { action: 'verify', target: 'Images', description: 'Verify all images load correctly' },
          { action: 'verify', target: 'Gallery Controls', description: 'Verify gallery navigation if present' },
          { action: 'verify', target: 'Image Quality', description: 'Verify images are clear and properly sized' }
        ],
        assertions: [
          'All images load without errors',
          'No broken image placeholders',
          'Gallery/slider navigation works if present',
          'Images display at correct resolution'
        ],
        elements: getElements('IMAGES').slice(0, 3).map(e => e.selector)
      });
    } else {
      // Generic media flow for other domains
      flows.push({
        name: 'Media Content Display',
        priority: 'Medium',
        description: 'Verify media content loads correctly',
        steps: [
          { action: 'navigate', target: 'Page with Media', description: 'Navigate to page with media content' },
          { action: 'verify', target: 'Media Element', description: 'Verify media element is visible' },
          { action: 'verify', target: 'Media Loaded', description: 'Verify media loads correctly' }
        ],
        assertions: [
          'Media element loads correctly',
          'Media displays without errors'
        ],
        elements: getElements('MEDIA').slice(0, 3).map(e => e.selector)
      });
    }
...
function generateFunctionalRequirements(analysisResult) {
  ...
  if (elementCategories.includes('MEDIA')) {
    // Domain-aware media requirements
    // For OTT/Streaming, media is critical; for other domains, it's supplementary
    const isOTT = websiteType?.type === 'OTT_STREAMING';
    const isRetailOrCorporate = ['RETAIL_STORE', 'CORPORATE', 'HEALTHCARE_PHARMA'].includes(websiteType?.type);

    if (isOTT) {
      // Full media playback requirements for OTT platforms
      requirements.push({
        id: `FR-${String(reqId++).padStart(3, '0')}`,
        feature: 'Media Content Playback',
        type: 'Media',
        priority: 'Critical',
        description: 'Video/Audio content shall load and play correctly with full playback controls',
        acceptanceCriteria: [
          'Media elements load without errors',
          'Playback controls (play/pause/seek) are functional',
          'Volume and fullscreen controls work',
          'Media plays without buffering issues',
          'Quality selection works if available'
        ],
        testable: true
      });
    } else if (isRetailOrCorporate) {
      // For retail/corporate, media is promotional - focus on images loading
      requirements.push({
        id: `FR-${String(reqId++).padStart(3, '0')}`,
        feature: 'Images and Gallery Display',
        type: 'Visual',
        priority: 'Medium',
        description: 'Product images and promotional media shall load correctly',
        acceptanceCriteria: [
          'All images load without errors',
          'No broken image placeholders',
          'Image galleries/sliders display correctly',
          'Promotional banners are visible'
        ],
        testable: true
      });
    } else {
      // Generic media requirement for other domains
      requirements.push({
        id: `FR-${String(reqId++).padStart(3, '0')}`,
        feature: 'Media Content',
        type: 'Media',
        priority: 'Medium',
        description: 'Media content shall load correctly',
        acceptanceCriteria: [
          'Media elements load without errors',
          'Basic playback works if applicable',

  ...
  return (
    <>
      ...
      <p className="sub">
        ...
        co-located with <code>npm run start:all</code>.
      </p>
...
npm start                      # API only  → http://localhost:3000
# or:
npm run start:all              # API + orchestrator + executor`}
      ...
      <Pipeline>
        ...
        <PipelineStage id="04" title="Build the UI">
          ...
          <code>dist/web/</code>, which the API serves.
        </PipelineStage>
        <PipelineStage id="05" title="Start the runtime">
          <code>npm start</code> boots the HTTP API only (no Chromium). For the full pipeline
          locally use <code>npm run start:all</code>, which launches the API, orchestrator, and
          executor together.
        </PipelineStage>
        <PipelineStage id="06" title="Verify">
          <code>curl http://localhost:3000/health</code> and open the UI. Run the smoke test with{' '}
...
function Compose() {
  return (
    <>
      ...
      <p className="sub">
        Three workspace-scoped app images (<code>zero-api</code>, <code>zero-orchestrator</code>,{' '}
        <code>zero-executor</code>) plus Postgres, Redis, and MinIO. Chromium lives only in the
        executor image.
      </p>
...
docker compose down -v             # stop + wipe pgdata / miniodata / artifacts`}
      ...
      <Pipeline>
        <PipelineStage id="01" title="Prerequisites">
          Docker Engine + Compose v2. Free ports 3000 / 5174 / 5175 / 5432 / 6379 / 9000 / 9001, or
          override with <code>APP_PORT</code>, <code>POSTGRES_PORT</code>, etc.
        </PipelineStage>
        <PipelineStage id="02" title="Build + start">
          <code>docker compose up --build</code> builds the API image (which bundles the Vite UI
          into <code>dist/web</code>), the orchestrator and executor images, then starts Postgres,
          Redis, MinIO, and <code>minio-init</code> (creates bucket <code>zero-artifacts</code>).
        </PipelineStage>
        <PipelineStage id="03" title="Shared state">
          All three app services share the <code>app-artifacts</code> volume at{' '}
        ...
        <PipelineStage id="04" title="Health checks">
          <code>curl localhost:3000/health</code> · <code>curl localhost:5175/status</code>. The
          workflow service probes the bind-mounted repo for milestone state.
        </PipelineStage>
...
function Hybrid() {
  return (
    <>
      ...
      <p className="sub">
        The fastest way to iterate on Node while still using real Postgres, Redis, and MinIO. No app
        image rebuilds.
      </p>
...
# optional MinIO S3 vars — see .env.example
...
npm run start:all                  # API + orchestrator + executor on the host`}
      ...
      <Pipeline>
        <PipelineStage id="01" title="Start infra only">
          <code>docker compose up -d postgres redis minio</code>. If host <code>:5432</code> is
          busy, <code>POSTGRES_PORT=15432 docker compose up -d postgres</code> and point{' '}
        ...
        <PipelineStage id="02" title="Point the host app at it">
          ...
          Add the MinIO <code>S3_*</code> vars only when testing the object store.
        </PipelineStage>
...
function Production() {
  return (
    <>
      ...
      <p className="sub">
        Same three images, hardened. Auth is forced on, secrets are required, and infrastructure uses
      ...
      <Pipeline>
        ...
        <PipelineStage id="03" title="Build + push images">
          Build <code>zero-api</code>, <code>zero-orchestrator</code>, and <code>zero-executor</code>{' '}
          (Playwright base) and push to your registry. Keep Playwright out of the API image.
        </PipelineStage>
        <PipelineStage id="04" title="Deploy the three tiers">
          Run the API behind TLS, the orchestrator as a long-lived worker, and the executor as an
          autoscaled job pool keyed on queue depth. Give the executor <code>shm_size ≥ 1gb</code>{' '}
          for Chromium.
        </PipelineStage>
        <PipelineStage id="05" title="Run migrations">
          Point <code>DATABASE_URL</code> at managed Postgres; tables initialize via{' '}
          <code>@zero/db</code> <code>initAllTables</code> (ER:{' '}
          <a href="#tech-schema">schema</a>). A forward/reversible <code>migrate</code> CLI is{' '}
          <StatusBadge status="not-done" /> — today it is <code>CREATE TABLE IF NOT EXISTS</code>.
        </PipelineStage>
        <PipelineStage id="06" title="Smoke + observe">
          Hit <code>/health</code> and <code>/api/health/detailed</code>, start one run, confirm
          artifacts land in the object store. Dashboards for <code>queue_lag</code> /{' '}
          <code>executor_slots_used</code> and a runbook + SLOs are{' '}
          <StatusBadge status="not-done" /> (see Ship Checklist).
        </PipelineStage>
      </Pipeline>

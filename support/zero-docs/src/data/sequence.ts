...
export const SEQUENCE: readonly SeqPhase[] = [
  {
    ...
    steps: [
      { n: 1, from: 'Client', to: 'API GW → API', op: 'POST /runs — metadata only', crosses: 'http', owner: '@zero/api', why: 'Only the small form fields travel here: URL, notes, options. No file bytes yet.' },
      ...
      { n: 3, from: 'API', to: 'Client', op: '202 { runId, uploads[] }', crosses: 'http', owner: '@zero/api', why: 'The browser now knows the run id and exactly where to put each file.' },
      ...
      { n: 7, from: 'API', to: 'Client', op: 'open SSE /runs/:id/stream', crosses: 'http', owner: '@zero/api', why: 'One long-lived channel, so the UI sees stage progress without polling.' },
    ],
  },
  {
    id: 'B',
  ...
  },
  {
    ...
    steps: [
      ...
      { n: 34, from: 'Client', to: 'API', op: 'GET /runs/:id/download', crosses: 'http', owner: '@zero/web', why: 'The user asks for the report bundle.' },

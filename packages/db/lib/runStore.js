...
/** Postgres rows omit optional stage artifacts; run.json on the volume is authoritative. */
const OPTIONAL_ARTIFACT_KEYS = [
  "webAnalysis",
  "accessibilityReport",
  "performanceReport",
  "securityReport"
...
async function mergeOptionalArtifactsFromFile(run, root) {
  if (!run || !run.id) return run;
  try {
    const data = await fs.readFile(path.join(root, run.id, "run.json"), "utf8");
    const fileRun = JSON.parse(data);
    const fileArts = fileRun.artifacts || {};
    run.artifacts = run.artifacts || {};
    for (const key of OPTIONAL_ARTIFACT_KEYS) {
      if (fileArts[key] != null && run.artifacts[key] == null) {
        run.artifacts[key] = fileArts[key];
      }
    }
  } catch {
    // run.json may not exist yet on a fresh run
  }
  return run;
}
...
function createRunStore(opts = {}) {
  ...
  async function getRun(id) {
    ...
    try {
      ...
      if (!row) return null;
      const run = await mergeOptionalArtifactsFromFile(rowToRun(row, root), root);
      memory.set(id, run);
      return run;
    } catch (err) {
